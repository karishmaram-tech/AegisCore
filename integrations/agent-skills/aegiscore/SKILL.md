...
You interact through the `aegiscore_*` MCP tools (listed below). The heavy
work runs inside the Aegiscore server; you are the operator at the console.

## Mental model — read this first

- An **engagement is a thread.** `aegiscore_start_engagement` returns a
...
- A Aegiscore **LangGraph server** must be running and reachable
  (`AEGISCORE_API_URL`, default `http://localhost:2024`). If a tool errors with
...
- The `aegiscore` MCP server must be registered and launched with
  `AEGISCORE_SKIP_BOOT=1` (fast start). See the integration docs.

## Tools

| Tool | Use it to | Key args | Returns (key fields) |
|------|-----------|----------|----------------------|
| `aegiscore_list_graphs` | see available graphs | — | `[{graph_id, name}]` |
| `aegiscore_list_engagements` | browse / resume | `limit` | `[{thread_id, engagement_name, status}]` |
| `aegiscore_start_engagement` | launch | `targets[]`, `instruction`, `scan_mode`, `engagement_name?` | `{thread_id, engagement_name, run_id, status}` |
| `aegiscore_transcript` | watch the narrative | `thread_id`, `after_index`, `limit` | `{messages[], next_index, total, run_status}` |
| `aegiscore_watch` | live sub-agent burst | `thread_id`, `max_seconds`, `max_events` | `{events[], run_status}` |
| `aegiscore_send_message` | steer / answer / `/model` | `thread_id`, `message` | `{run_id, status}` |
| `aegiscore_engagement_state` | OPPLAN / scope / phase | `thread_id` | `{engagement_name, message_count, values}` |
| `aegiscore_engagement_status` | run status + findings ready | `thread_id`, `engagement_name?` | `{status, findings_available}` |
| `aegiscore_engagement_findings` | pull results | `engagement_name`, `include_sarif?` | `{available, result_count, level_counts, sarif?}` |
| `aegiscore_cancel_engagement` | stop the run | `thread_id` | text |
...
1. **Pick a graph.** Usually `aegiscore` (full kill chain). Use `recon` for
   recon-only, `soundwave` for planning. `aegiscore_list_graphs()` if unsure.
2. **Start.** `aegiscore_start_engagement(targets=[…], instruction="In scope: …;
   Out of scope: …", scan_mode="standard")`. Save `thread_id` **and**
   `engagement_name`.
3. **Watch + narrate.** Loop `aegiscore_transcript(thread_id,
   ...
   a live burst use `aegiscore_watch(thread_id)`. Check
   `aegiscore_engagement_status(thread_id, engagement_name)`; stop polling when
   `status` is terminal **or** `findings_available` is true.
4. **Steer** when useful: `aegiscore_send_message(thread_id, "skip the staging
   host, focus on the API")`, answer the coordinator, or switch models with
   `/model anthropic/claude-opus-4-8`.
5. **Report.** When findings exist:
   `aegiscore_engagement_findings(engagement_name, include_sarif=true)` →
   present severity, counts, and reproduction. `aegiscore_engagement_state` for
   the OPPLAN/phase.
6. **Resume later.** `aegiscore_list_engagements()` → reuse any `thread_id`.
...
- Use the returned `next_index` as your cursor so each update covers only new
  activity.
- `aegiscore_watch` blocks up to `max_seconds` (≤45) — use it for a quick live
...
- **Connection failure** → the Aegiscore server isn't up at `AEGISCORE_API_URL`.
