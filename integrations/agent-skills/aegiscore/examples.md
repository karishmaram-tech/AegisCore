...
```
aegiscore_transcript(thread_id="th_1", after_index=0) -> { next_index: 6, run_status: "running",
...
```
aegiscore_engagement_status(thread_id="th_1", engagement_name="mcp-20260603-2140")
  -> { status: "running", findings_available: true }
aegiscore_engagement_findings(engagement_name="mcp-20260603-2140", include_sarif=true)
...
```
aegiscore_start_engagement(targets=["10.10.0.0/24"],
...
`aegiscore_engagement_state(thread_id)` (`values` holds the recon map).
...
```
aegiscore_send_message(thread_id="th_1",
...
```
aegiscore_send_message(thread_id="th_1", message="/model anthropic/claude-opus-4-8")
```
...
```
aegiscore_list_engagements(limit=20) -> [{ thread_id: "th_1", engagement_name: "mcp-20260603-2140", status: "idle" }, …]
aegiscore_transcript(thread_id="th_1", after_index=0, limit=10)   # recent narrative
aegiscore_engagement_findings(engagement_name="mcp-20260603-2140")  # what was found
```

Summarise where it stands + findings. To continue, `aegiscore_send_message` or
`aegiscore_start_engagement` a fresh pass.

## 5. A live burst (when the user wants detail "right now")

```
aegiscore_watch(thread_id="th_1", max_seconds=15, max_events=30)
...
```
aegiscore_engagement_status(thread_id="th_1", engagement_name="…") -> { status: "error" }
aegiscore_transcript(thread_id="th_1", after_index=<n>, limit=10)  # read the cause
```

Tell the user what failed (from the last messages) and offer to
`aegiscore_send_message` a correction or start a fresh engagement. If a tool
