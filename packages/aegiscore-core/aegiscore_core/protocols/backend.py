...
The Aegiscore framework composes a ``CompositeBackend`` per agent
(``aegiscore.backends.make_agent_backend``) that routes ``/skills/``
...
class BackendProtocol(Protocol):
    """Duck-type contract for a Aegiscore filesystem backend.
