...
Submodules:
  ...
  * ``slots`` — middleware slot enum and per-role applicability mapping.
    Was ``aegiscore.agents.middleware_slots``; framework keeps the
...
``aegiscore_core.plugin_loader`` after Phase 1.B).
"""

from __future__ import annotations

from aegiscore_core.contracts import contributions, slots
