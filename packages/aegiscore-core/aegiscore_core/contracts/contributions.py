...
``aegiscore_core.plugin_loader``) keeps a back-compat shape during
...
class PromptContribution:
    ...
    directly under the new ``aegiscore.prompts`` entry-point group.
...
class SubAgentContribution:
    ...
    in ``aegiscore_core.plugin_loader``. The ``items`` carry
    ...
    def __post_init__(self) -> None:
        if not self.parent_agents:
            raise ValueError(
                ...
                "Pass parent_agents=('aegiscore',) etc."
            )
...
class SafetyDeclaration:
    ...
    ``SAFETY_CRITICAL_SLOTS`` from ``aegiscore-core``.
