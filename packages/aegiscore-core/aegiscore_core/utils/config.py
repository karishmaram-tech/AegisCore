"""Aegiscore configuration — defaults + environment variable overrides.

LLM model assignments are defined in aegiscore_core.types.llm (LLMModelMapping).
...
``aegiscore.backends.factory.build_sandbox_backend`` — no schema field needed.
...
from aegiscore_core.types.llm import ModelProfile
...
_LOCAL_PROXY_KEY = "sk-aegiscore-master"  # nosemgrep: aegiscore-no-hardcoded-default-key
