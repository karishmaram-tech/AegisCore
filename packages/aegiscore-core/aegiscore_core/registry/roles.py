...
``aegiscore_core.contracts.slots.SLOTS_PER_ROLE``. Plugins register
custom roles via the new ``aegiscore.roles`` entry-point group;
this closes gap §8 #5 (a vendor ``apt`` previously had to abuse
``default_role="aegiscore"`` in ``LLMFactory``).
