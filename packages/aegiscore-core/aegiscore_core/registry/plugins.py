...
``PluginRegistry.load()`` walks the eight Aegiscore entry-point
...
from aegiscore_core.registry.resolution import RoleResolution
...
# with ``aegiscore_core.plugin_loader``'s *_GROUP constants — the
...
class PluginRegistry:
    ...
    def load(cls) -> PluginRegistry:
        ...
        Walks every Aegiscore entry-point group via
    ...
    def set_role_resolution(cls, role: str, resolution: RoleResolution) -> None:
        ...
        Called by ``aegiscore._boot.run()`` after registering OSS
        roles. ``aegiscore-core`` itself can't compose the resolution
    ...
    def introspect_role(self, role: str) -> RoleResolution | None:
        ...
        ``aegiscore._boot.run()`` (or by any caller using
