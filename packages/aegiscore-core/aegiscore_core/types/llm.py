...
User has [anthropic_api, openai_api]. Profile=eco.
  aegiscore (HIGH) → primary=anthropic/claude-opus-4-8, fallback=openai/gpt-5.5
...
User has [anthropic_oauth, anthropic_api]. Profile=eco.
  aegiscore (HIGH) → primary=auth/claude-opus-4-8, fallback=anthropic/claude-opus-4-8
  (OAuth subscription primary, paid API as fallback when subscription quota hits.)

User has [anthropic_oauth] only. Profile=eco.
  aegiscore (HIGH) → primary=auth/claude-opus-4-8, fallback=None

User has [openai_api]. Profile=eco.
  aegiscore (HIGH) → primary=openai/gpt-5.5, fallback=None
...
METHOD_MODELS: dict[AuthMethod, dict[Tier, str]] = {
    ...
    AuthMethod.OLLAMA_LOCAL: {
        ...
        # endpoint that supports tool/function calling, which Aegiscore
    ...
    # endpoint which silently drops the ``tools`` parameter; Aegiscore
...
def _resolve_ollama_model() -> str | None:
    ...
    one that exposes tool/function calling, which every Aegiscore agent
    relies on.
...
_PROXY_DEFAULT_LOCAL_KEY = "sk-aegiscore-master"  # nosemgrep: aegiscore-no-hardcoded-default-key
...
class LLMModelMapping(BaseModel):
    ...
    def get_assignment(self, role: str, *, default_role: str | None = None) -> ModelAssignment:
        ...
        Plugin orchestrators with a custom role (e.g. ``"aegiscore-pro"``)
        that aren't part of the OSS ``AGENT_TIERS`` registry can pass
        ``default_role="aegiscore"`` (or any OSS role) to inherit that
