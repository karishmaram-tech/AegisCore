...
# carries the MITRE-group-ID-keyed adversary persona that Aegiscore's
...
# For Aegiscore's autonomous flow the agent only needs the operator
...
class EngagementBundle(BaseModel):
    ...
    (Aegiscore) generates ``opplan`` separately by reading the
