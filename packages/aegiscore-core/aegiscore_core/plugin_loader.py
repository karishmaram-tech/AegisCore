"""Plugin discovery for Aegiscore.

Aegiscore supports adding tools, middleware, agents, and callback handlers
...
TOOLS_GROUP = "aegiscore.tools"
MIDDLEWARE_GROUP = "aegiscore.middleware"
AGENTS_GROUP = "aegiscore.agents"
SUBAGENTS_GROUP = "aegiscore.subagents"
CALLBACKS_GROUP = "aegiscore.callbacks"
SKILLS_GROUP = "aegiscore.skills"
PROMPTS_GROUP = "aegiscore.prompts"  # Spec §7.2 Principle 7 — prompt-only plugins
ROLES_GROUP = "aegiscore.roles"  # Spec §7.2 Principle 4 — custom agent roles
...
def _config_file_bundles() -> frozenset[str] | None:
    ...
    Lookup order (first match wins):
      1. ``.aegiscore.toml`` →  ``[plugins] enabled = [...]``
      2. ``pyproject.toml``   →  ``[tool.aegiscore.plugins] enabled = [...]``
    ...
    if pyproject.is_file():
        try:
            ...
            value = data.get("tool", {}).get("aegiscore", {}).get("plugins", {}).get("enabled")
            if value is not None:
                normalized = _normalize_bundles_value(value)
                if normalized is not None:
                    return normalized
...
class PluginBundle:
    ...
    ``aegiscore.agents.middleware_slots``.
...
class SubAgentSpec:
    ...
    via the ``aegiscore.subagents`` entry-point group. The main agent
    ...
    Fields:
        ...
        factory: zero-arg callable returning the compiled subagent (e.g.
            ``aegiscore.agents.standard.recon.create_recon_agent``). The
        ...
        parent_agents: tuple of main-agent names this subagent should be
            attached to (e.g. ``("aegiscore",)`` or
            ``("aegiscore", "vulnresearch")``).
    ...
    # ``aegiscore_core.protocols.agent``; the import is local to keep
...
def load_plugin_skill_sources(role: str | None = None) -> list[str]:
    ...
    static ``list[str]`` under the ``aegiscore.skills`` entry-point group.
...
def load_plugin_prompts(role: str) -> list[Any]:
    ...
    contributions via the ``aegiscore.prompts`` entry-point group,
...
def load_plugin_role_specs() -> list[Any]:
    """Discover custom role specs contributed under ``aegiscore.roles``.
...
def _discover_subagent_specs() -> list[SubAgentSpec]:
    """Discover every ``SubAgentSpec`` exported under ``aegiscore.subagents``."""
...
def load_plugin_agents() -> dict[str, str]:
    ...
    matching how OSS agents are wired (``aegiscore/agents/recon.py:graph``).
    """
    found: dict[str, str] = {}
    try:
