...
from aegiscore_core.registry import plugins as plugins_module
...
def test_same_name_in_same_group_from_different_packages_collides(
    ...
    ``aegiscore.tools`` must surface a PluginConflictWarning."""
    _patch_entry_points(
        monkeypatch,
        {
            "aegiscore.tools": [
...
def test_same_name_same_package_does_not_collide(
    ...
    assert plugins[0].groups == ("aegiscore.middleware", "aegiscore.tools")
