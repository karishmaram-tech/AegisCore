...
``aegiscore-core`` is the contract layer. By design it must NEVER
...
imports every submodule of ``aegiscore_core`` and reports any
...
import aegiscore_core

before = set(sys.modules)
for module_info in pkgutil.walk_packages(
    aegiscore_core.__path__,
    prefix='aegiscore_core.',
):
...
# Account for the modules pulled in by `import aegiscore_core` itself
...
def test_core_has_no_runtime_deps() -> None:
    """Spec §11.4 — aegiscore_core must not transitively pull in
    langchain / langgraph / deepagents / httpx / fastapi.

    Defends the contract layer's pure-pydantic-plus-stdlib promise so
    plugin authors that depend on ``aegiscore-core`` only never
    ...
    assert result.returncode == 0, (
        f"aegiscore-core leaked runtime imports for {result.stdout.strip()!r}; "
