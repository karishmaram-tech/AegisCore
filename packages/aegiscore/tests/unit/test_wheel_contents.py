...
def test_wheel_bundles_skills_but_excludes_benchmark(tmp_path):
    ...
    assert any(n.startswith("aegiscore/skills/standard/") for n in names), (
        "standard skills missing from wheel"
    )
    assert any(n.startswith("aegiscore/skills/shared/") for n in names), (
        "shared skills missing from wheel"
    )
    assert any(n.startswith("aegiscore/skills/plugins/") for n in names), (
        "plugins skills missing from wheel"
    )
    assert not any(n.startswith("aegiscore/skills/benchmark/") for n in names), (
