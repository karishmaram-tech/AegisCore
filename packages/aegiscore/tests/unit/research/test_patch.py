...
from aegiscore.tools.research.patch import patch_propose, patch_verify
...
def _configure_kg(monkeypatch):
    fake = _FakeStore()
    monkeypatch.setattr("aegiscore.tools.research._state._store", fake)
    return fake
...
class TestPatchVerify:
    async def test_verified_flips_severity_to_info(
        ...
        bash_mod = importlib.import_module("aegiscore.tools.bash.bash")
