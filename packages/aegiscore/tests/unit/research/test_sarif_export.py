"""Tests for aegiscore.tools.research.sarif_export."""
...
from aegiscore.tools.research.sarif_export import (
...
def test_export_emits_v2_1_0_envelope():
    ...
    assert doc["runs"][0]["tool"]["driver"]["name"] == "Aegiscore"


def test_export_empty_graph_yields_zero_results():
    doc = export_findings_to_sarif(_FakeGraph([]))
    assert doc["runs"][0]["results"] == []
    assert doc["runs"][0]["properties"]["aegiscore-finding-count"] == 0
...
def test_export_groups_results_by_rule_id():
    ...
    assert rule_ids == ["aegiscore/sqli", "aegiscore/xss"]
...
def test_export_rule_id_prefers_cve_over_cwe():
    ...
    doc = export_findings_to_sarif(graph)
    assert doc["runs"][0]["results"][0]["ruleId"] == "aegiscore/CVE-2024-12345"
...
def test_export_rule_id_falls_back_to_technique_then_label():
    ...
    assert ids == ["aegiscore/T1190", "aegiscore/my-finding-name"]
...
def test_write_sarif_round_trip(tmp_path: Path):
    ...
    assert doc["runs"][0]["properties"]["aegiscore-engagement"] == "testing"
