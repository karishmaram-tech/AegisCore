...
from aegiscore.tools.research.cve import (
    Exploitability,
    _Cache,
    _parse_epss,
    _parse_nvd,
...
class TestCacheLRUEviction:
    def test_evicts_oldest_when_over_capacity(self, tmp_path: Path) -> None:
        ...
        with patch("aegiscore.tools.research.cve.MAX_CACHE_ENTRIES", 3):
...
class TestLookupCve:
    ...
    def test_network_response_builds_exploitability(self, tmp_path: Path) -> None:
        ...
        async def _run() -> Exploitability:
            with (
                patch("aegiscore.tools.research.cve._fetch_nvd", new_callable=AsyncMock) as fn,
                patch("aegiscore.tools.research.cve._fetch_epss", new_callable=AsyncMock) as fe,
            ):
...
class TestLookupPackage:
    def test_returns_cve_ids_from_osv(self) -> None:
        ...
        async def _run() -> list[str]:
            with patch(
                "aegiscore.tools.research.cve._fetch_osv", new_callable=AsyncMock
            ) as mock_osv:
