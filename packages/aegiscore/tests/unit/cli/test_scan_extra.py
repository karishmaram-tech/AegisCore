"""Additional coverage for aegiscore.cli.scan — uncovered paths from test_scan.py.
...
from aegiscore.cli.scan import (
    EXIT_CONFIG,
    EXIT_FINDINGS,
    EXIT_INTERNAL,
    EXIT_OK,
...
def test_dispatch_scan_raises_runtime_error_when_sdk_missing() -> None:
    from aegiscore.cli.scan import _dispatch_scan_via_sdk
...
def test_load_findings_graph_env_var_workspace_used(
    ...
    with patch.dict(
        sys.modules,
        {
            "aegiscore_core": mock_module,
            "aegiscore_core.types": mock_module,
            "aegiscore_core.types.kg": mock_module,
        },
    ):
        result = _load_findings_graph("my-engagement")
...
def test_main_diff_scope_fallback_on_git_failure(
    ...
    with (
        patch("aegiscore.cli.scan._git_diff_files", return_value=None),
        patch("aegiscore.cli.scan._dispatch_scan_via_sdk", return_value=mock_sdk_result),
        patch("aegiscore.cli.scan._load_findings_graph", return_value=None),
    ):
        rc = main(
            [
                "--target",
...
def test_main_sdk_runtime_error_returns_config_exit(
    ...
    with patch(
        "aegiscore.cli.scan._dispatch_scan_via_sdk",
...
def test_main_with_findings_triggers_exit_findings(
    ...
    with (
        patch("aegiscore.cli.scan._dispatch_scan_via_sdk", return_value=mock_sdk_result),
        patch("aegiscore.cli.scan._load_findings_graph", return_value=_Graph()),
    ):
        rc = main(
            [
                "--target",
...
def test_main_engagement_name_override(tmp_path: Path, capsys: pytest.CaptureFixture) -> None:
    ...
    with (
        patch("aegiscore.cli.scan._dispatch_scan_via_sdk", side_effect=_fake_dispatch),
        patch("aegiscore.cli.scan._load_findings_graph", return_value=None),
    ):
...
def test_main_diff_scope_passes_diff_files_to_sdk(
    ...
    with (
        patch("aegiscore.cli.scan._git_diff_files", return_value=diff_result),
        patch("aegiscore.cli.scan._dispatch_scan_via_sdk", side_effect=_fake_dispatch),
        patch("aegiscore.cli.scan._load_findings_graph", return_value=None),
    ):
        rc = main(
