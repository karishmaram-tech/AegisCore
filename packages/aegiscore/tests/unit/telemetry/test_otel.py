"""Tests for ``aegiscore.telemetry.otel`` — opt-in OTLP trace export.
...
hierarchy with the documented Aegiscore attributes. Tests install an
...
from aegiscore.telemetry import otel as otel_module  # noqa: E402
from aegiscore.telemetry.otel import (  # noqa: E402
    record_llm_cost,
...
_SHARED_PROVIDER = TracerProvider(resource=Resource.create({"service.name": "aegiscore-test"}))
...
def memory_exporter(monkeypatch: pytest.MonkeyPatch) -> Iterator[InMemorySpanExporter]:
    ...
    otel_module._TRACER = trace.get_tracer("aegiscore")
...
def test_disabled_by_default_emits_no_spans(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    with start_engagement_span("eng-x"):
        with start_agent_span("aegiscore"):
...
def test_engagement_agent_tool_llm_hierarchy(memory_exporter: InMemorySpanExporter) -> None:
    ...
    assert "aegiscore.engagement" in spans
    assert "aegiscore.agent_run" in spans
    assert "aegiscore.tool_call" in spans
    assert "aegiscore.llm_call" in spans
    engagement = spans["aegiscore.engagement"]
    agent = spans["aegiscore.agent_run"]
    tool = spans["aegiscore.tool_call"]
    llm = spans["aegiscore.llm_call"]
...
def test_attributes_are_set(memory_exporter: InMemorySpanExporter) -> None:
    ...
    spans = {span.name: span for span in memory_exporter.get_finished_spans()}
    engagement = _attrs(spans["aegiscore.engagement"])
    agent = _attrs(spans["aegiscore.agent_run"])
    tool = _attrs(spans["aegiscore.tool_call"])
    llm = _attrs(spans["aegiscore.llm_call"])
    assert engagement["aegiscore.engagement_id"] == "eng-1"
    assert agent["aegiscore.agent"] == "aegiscore"
    assert agent["aegiscore.engagement_id"] == "eng-1"
    assert tool["aegiscore.tool"] == "update_objective"
    assert tool["aegiscore.opplan.objective_id"] == "OBJ-001"
    assert tool["aegiscore.engagement_id"] == "eng-1"
    assert llm["aegiscore.llm.model"] == "anthropic/claude-haiku-4-5"
    assert llm["aegiscore.llm.prompt_tokens"] == 120
    assert llm["aegiscore.llm.completion_tokens"] == 45
    assert llm["aegiscore.llm.cost_usd"] == pytest.approx(0.123456)
    assert llm["aegiscore.opplan.objective_id"] == "OBJ-001"
...
def test_record_llm_cost_falls_back_to_engagement_span(
    ...
    with start_engagement_span("eng-2"):
        record_llm_cost(2.5)
    eng = next(s for s in memory_exporter.get_finished_spans() if s.name == "aegiscore.engagement")
    assert _attrs(eng)["aegiscore.llm.cost_usd"] == pytest.approx(2.5)
...
def test_record_llm_cost_falls_back_when_llm_span_write_raises(
    ...
    eng = next(s for s in memory_exporter.get_finished_spans() if s.name == "aegiscore.engagement")
    assert _attrs(eng)["aegiscore.llm.cost_usd"] == pytest.approx(3.5)
...
def test_record_llm_cost_ignores_none(memory_exporter: InMemorySpanExporter) -> None:
    ...
    llm = next(s for s in memory_exporter.get_finished_spans() if s.name == "aegiscore.llm_call")
    assert "aegiscore.llm.cost_usd" not in _attrs(llm)
