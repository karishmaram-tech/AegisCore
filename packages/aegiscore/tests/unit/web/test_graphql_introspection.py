...
from aegiscore.tools.web.graphql import GraphQLSchema, _unwrap_type
...
def test_from_introspection_missing_schema_returns_empty_and_warns(
    ...
    # The ``aegiscore`` logger is configured with propagate=False, so caplog
    # (which listens on the root logger) won't see records unless we re-enable
    # propagation for the duration of the call.
    decep = logging.getLogger("aegiscore")
    prev = decep.propagate
    decep.propagate = True
    try:
        with caplog.at_level(logging.WARNING, logger="aegiscore.tools.web.graphql"):
