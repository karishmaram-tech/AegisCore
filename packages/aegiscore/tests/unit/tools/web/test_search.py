...
from aegiscore.middleware.roe import GATED_TOOL_NAMES, NETWORK_TARGET_EXTRACTORS
from aegiscore.middleware.untrusted_output import UNTRUSTED_TOOL_NAMES
from aegiscore.tools.web import search
...
async def test_web_fetch_ok(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert "aegiscore.sandbox_web fetch" in cmd
...
async def test_web_search_hits(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert "aegiscore.sandbox_web search" in fake.commands[0]
