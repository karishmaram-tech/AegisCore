"""Tests for ``aegiscore.tools.mcp.client``.
...
from aegiscore.tools.mcp.client import (
    ENV_SERVERS,
    MCPServerConfig,
    load_mcp_tools,
...
def _propagate_mcp_logger():
    """Let ``caplog`` see records — the ``aegiscore`` root sets
    ``propagate=False`` (see ``aegiscore_core.utils.logging``).
    """
    # The "aegiscore" root logger has ``propagate=False`` — records
...
def test_parse_malformed_json_returns_empty_and_warns(caplog: pytest.LogCaptureFixture):
    with caplog.at_level(logging.WARNING, logger="aegiscore.tools.mcp"):
...
async def test_load_mcp_tools_retries_transient_failures(
    ...
    from aegiscore.tools.mcp import client as mcp_client
