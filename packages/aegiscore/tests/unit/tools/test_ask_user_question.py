"""Unit tests for ``aegiscore.tools.interaction.ask_user_question``."""
...
from aegiscore.tools.interaction import ask_user_question
...
def test_emits_custom_event_with_id_and_payload():
    ...
    with (
        patch(
            "aegiscore.tools.interaction.ask_user.get_stream_writer",
            return_value=fake_writer,
        ),
        patch(
            "aegiscore.tools.interaction.ask_user.interrupt",
            return_value="Yes",
        ),
    ):
        result = _invoke()
