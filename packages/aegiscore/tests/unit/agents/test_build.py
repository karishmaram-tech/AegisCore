...
from aegiscore.agents import build as build_module
from aegiscore.agents.middleware_slots import SAFETY_CRITICAL_SLOTS, MiddlewareSlot
...
from aegiscore.middleware.hitl import HITLApprovalMiddleware
from aegiscore.middleware.prompt_injection_shield import PromptInjectionShieldMiddleware
from aegiscore.middleware.untrusted_output import UNTRUSTED_TOOL_NAMES
from aegiscore_core import plugin_loader
from aegiscore_core.plugin_loader import PluginBundle
...
def test_plugin_bundle_unrestricted_matches_every_role():
    ...
    assert bundle.matches_role("aegiscore")
    ...
    with patch.object(build_module, "entry_points", return_value=[]):
        with patch.object(plugin_loader, "entry_points", return_value=[]):
            result = build_module.build_middleware(
                role="aegiscore-pro",  # not in SLOTS_PER_ROLE
...
# ── aegiscore.skills entry-point group ───────────────────────────────
...
def test_skills_sources_appends_plugin_paths():
    ...
    from aegiscore.agents.middleware_slots import skills_sources_for
...
def test_llm_mapping_get_assignment_default_role_fallback():
    ...
    from aegiscore_core.types.llm import LLMModelMapping, ModelAssignment

    mapping = LLMModelMapping(
        assignments={
            "aegiscore": ModelAssignment(primary="openai/gpt-5", fallbacks=[], temperature=0.0)
        }
    )
    # Unknown role + default_role → returns aegiscore's assignment.
    assignment = mapping.get_assignment("aegiscore-pro", default_role="aegiscore")
    ...
    with pytest.raises(KeyError, match="No model assignment for role"):
        mapping.get_assignment("aegiscore-pro")
