...
half-wires: the ``aegiscore.subagents`` entry-point group (what the
orchestrator can dispatch), the LangGraph manifests (what can be served
as a standalone graph), and the role tables in ``aegiscore_core``
...
import aegiscore
from aegiscore.graph_registry import BUILTIN_GRAPHS, STANDARD_GRAPHS
from aegiscore.middleware.skillogy import _PHASE_FOR_ROLE
from aegiscore.skillogy.builder.seeds import load_asset_types
from aegiscore_core.contracts.slots import SLOTS_PER_ROLE
from aegiscore_core.types.llm import AGENT_TIERS
...
def _subagent_entry_point_names() -> list[str]:
    return sorted(ep.name for ep in entry_points(group="aegiscore.subagents"))


def test_every_registered_subagent_has_a_graph():
    """Each ``aegiscore.subagents`` entry point must have a built-in graph.
...
_PHASES_YAML = (
    Path(aegiscore.__file__).resolve().parent / "skillogy" / "builder" / "seeds" / "phases.yaml"
)
...
_MOC_YAML = (
    Path(aegiscore.__file__).resolve().parent / "skillogy" / "builder" / "seeds" / "moc.yaml"
)
