...
from aegiscore.agents import build
from aegiscore.agents.standard import blue_cell as agent_mod
...
def test_subagent_spec_targets_orchestrator() -> None:
    ...
    assert spec.parent_agents == ("aegiscore",)
...
def test_langgraph_registers_blue_cell_graph() -> None:
    ...
    assert (
        ...
        == "./packages/aegiscore/aegiscore/agents/standard/blue_cell.py:graph"
    )
...
def test_blue_cell_published_as_decepticon_subagent() -> None:
    ...
    pyproject = _REPO_ROOT / "packages" / "aegiscore" / "pyproject.toml"
    data = tomllib.loads(pyproject.read_text(encoding="utf-8"))
    subagents = data["project"]["entry-points"]["aegiscore.subagents"]
    assert subagents.get("blue_cell") == "aegiscore.agents.standard.blue_cell:SUBAGENT_SPEC"
