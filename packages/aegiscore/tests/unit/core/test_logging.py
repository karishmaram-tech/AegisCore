"""Unit tests for aegiscore_core.utils.logging."""
...
# Import the canonical module directly (not the aegiscore.core.logging
...
from aegiscore_core.utils import logging as dlog


@pytest.fixture(autouse=True)
def reset_logger():
    """Reset the aegiscore root logger between tests."""
    yield
...
def _capture(level: str | int = "INFO", fmt: str = "text") -> io.StringIO:
    ...
    handler = logging.getLogger("aegiscore").handlers[0]
    handler.stream = buf
    return buf
...
class TestGetLogger:
    ...
    def test_auto_configures_on_first_use(self) -> None:
        ...
        assert logging.getLogger("aegiscore").handlers, "handler should be installed"
        del log
...
class TestJsonFormat:
    def test_json_output_is_parseable(self) -> None:
        ...
        assert record["logger"] == "aegiscore.test"
        assert "ts" in record
...
class TestConfigureLogging:
    ...
    def test_env_var_override(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        from aegiscore_core.utils.logging import _JsonFormatter
