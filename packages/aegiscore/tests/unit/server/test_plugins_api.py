...
``aegiscore/server/plugins_api.py`` is mounted into the LangGraph
...
from aegiscore.graph_registry import _BUNDLE_TO_GRAPHS
from aegiscore.server import plugins_api
from aegiscore.server.plugins_api import (
    _path_to_module,
    _resolve_bundle,
    app,
)
...
def test_path_to_module_happy_path() -> None:
    module, variable = _path_to_module("./aegiscore/agents/plugins/vulnresearch.py:graph")
    assert module == "aegiscore.agents.plugins.vulnresearch"
    assert variable == "graph"


def test_path_to_module_strips_leading_dot_slash_and_py_suffix() -> None:
    module, variable = _path_to_module("aegiscore/agents/standard/recon.py:graph")
    assert module == "aegiscore.agents.standard.recon"
    assert variable == "graph"
...
def test_path_to_module_missing_variable_suffix_raises_value_error() -> None:
    with pytest.raises(ValueError, match="missing ':variable' suffix"):
        _path_to_module("./aegiscore/agents/plugins/vulnresearch.py")
