...
from aegiscore.tools.reporting.sarif import (
    SARIF_SCHEMA,
    SARIF_VERSION,
...
from aegiscore.tools.reporting.tools import report_sarif
...
class TestRenderSarifShape:
    def test_top_level_structure_is_sarif_2_1_0(self) -> None:
        ...
        run = payload["runs"][0]
        assert run["tool"]["driver"]["name"] == "Aegiscore"
...
class TestReportSarifTool:
    def test_writes_sarif_to_disk_and_reports_metadata(self, tmp_path: Path) -> None:
        out = tmp_path / "out" / "aegiscore.sarif"
        g = _seeded_graph()
        with patch("aegiscore.tools.reporting.tools._load", return_value=(g, None)):
            result = asyncio.run(
    ...
    def test_creates_parent_directories(self, tmp_path: Path) -> None:
        out = tmp_path / "nested" / "deeper" / "aegiscore.sarif"
        g = _seeded_graph()
        with patch("aegiscore.tools.reporting.tools._load", return_value=(g, None)):
