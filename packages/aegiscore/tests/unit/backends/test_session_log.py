...
from aegiscore.sandbox_kernel import BackgroundJobTracker, TmuxSessionManager
from aegiscore.sandbox_kernel.daemon import DaemonSandbox


def test_initialize_does_not_create_root_workspace_sessions_log():
    mgr = TmuxSessionManager("scan-1", "aegiscore-sandbox")
    TmuxSessionManager._initialized.discard("scan-1")

    with (
        patch.object(mgr, "_tmux") as mock_tmux,
        patch("aegiscore.sandbox_kernel.base.subprocess.run") as mock_run,
        patch("time.sleep"),
    ):
...
def test_read_screen_handles_capture_timeout_gracefully():
    ...
    mgr = TmuxSessionManager("hung", "aegiscore-sandbox")
...
def test_execute_async_poll_loop_capture_timeout_continues():
    ...
    mgr = TmuxSessionManager("flaky", "aegiscore-sandbox")
    TmuxSessionManager._initialized.add("flaky")
    ...
    with (
        ...
        patch("aegiscore.sandbox_kernel.tmux.POLL_INTERVAL", 0.01),
    ):
...
def test_poll_loop_capture_errors_dont_falsely_trigger_stall_detection():
    ...
    mgr = TmuxSessionManager("loaded", "aegiscore-sandbox")
    ...
    with (
        ...
        patch.object(mgr, "initialize", return_value=None),
        patch("aegiscore.sandbox_kernel.tmux.POLL_INTERVAL", 0.01),
        patch("aegiscore.sandbox_kernel.tmux.STALL_SECONDS", 0.05),
    ):
        result = asyncio.run(mgr.execute_async(command="ls", is_input=False, timeout=2))
...
def test_initialize_recreates_stale_cached_pane_without_error_string_matching():
    mgr = TmuxSessionManager("stale", "aegiscore-sandbox")
    mgr._pane_id = "%old"
...
def test_execute_recovers_initial_capture_failure_without_error_string_matching():
    mgr = TmuxSessionManager("recover", "aegiscore-sandbox")
    mgr._pane_id = "%bad"
