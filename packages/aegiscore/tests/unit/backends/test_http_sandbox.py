...
from aegiscore.backends.http_sandbox import HTTPSandbox, SandboxError, _retry_on_connection_error
from aegiscore.sandbox_kernel import BackgroundJob
...
def test_http_lazy_client_created_once_and_cached() -> None:
    ...
    assert "aegiscore-http-sandbox/1" in c1.headers["user-agent"]
...
def test_retry_succeeds_first_try_no_sleep(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(
        "aegiscore.backends.http_sandbox.time.sleep", lambda d: sleep_calls.append(d)
    )
...
def test_retry_exhausts_reraises_connect_timeout(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("aegiscore.backends.http_sandbox.time.sleep", lambda d: None)
