"""Tests for aegiscore.mcp_server.config (env-driven settings)."""

from __future__ import annotations

import pytest

from aegiscore.mcp_server.config import (
    ENV_API_URL,
    ENV_ASSISTANT,
    ENV_AUDIENCE,
    ENV_AUTH,
...
def test_load_config_defaults(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert cfg.default_assistant == "aegiscore"
...
def test_load_config_reads_jwt_auth_env(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setenv(ENV_AUDIENCE, "aegiscore-mcp")
    ...
    assert cfg.audience == "aegiscore-mcp"
