"""Tests for aegiscore.mcp_server.auth — the bearer gate on the HTTP transport."""
...
from aegiscore.mcp_server.auth import (  # noqa: E402
    build_auth,
    is_loopback_host,
    open_bind_error,
    resolve_auth_mode,
)
from aegiscore.mcp_server.config import ServerConfig  # noqa: E402


def _cfg(**kw: object) -> ServerConfig:
    base: dict[str, object] = {
        "langgraph_url": "http://localhost:2024",
        "default_assistant": "aegiscore",
...
def test_shared_secret_accepts_exact_token() -> None:
    ...
    assert access.client_id == "aegiscore-mcp:shared-secret"
...
def _jwt_cfg(pub: str, **kw: object) -> ServerConfig:
    return _cfg(issuer="https://issuer/", audience="aegiscore-mcp", public_key=pub, **kw)
...
def test_jwt_accepts_valid_token_and_reads_scopes(rsa_keys: tuple[str, str]) -> None:
    ...
    token = jwt.encode(
        {"iss": "https://issuer/", "aud": "aegiscore-mcp", "sub": "agent", "scope": "engage read"},
        priv,
        algorithm="RS256",
    )
...
def test_jwt_rejects_wrong_issuer(rsa_keys: tuple[str, str]) -> None:
    ...
    assert verifier is not None
    token = jwt.encode(
        {"iss": "https://evil/", "aud": "aegiscore-mcp", "sub": "a"}, priv, algorithm="RS256"
    )
    assert asyncio.run(verifier.verify_token(token)) is None
...
def test_build_server_installs_auth_middleware_when_secret_set() -> None:
    from aegiscore.mcp_server.server import build_server
