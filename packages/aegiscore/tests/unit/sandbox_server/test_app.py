...
The daemon (``aegiscore/sandbox_server/app.py``) is a thin transport
...
# Import the module object explicitly. ``aegiscore.sandbox_server.__init__``
# binds ``app`` (the FastAPI instance) as a package attribute, so a plain
# ``import aegiscore.sandbox_server.app as app_module`` would resolve
# ``.app`` to that instance rather than the submodule. ``import_module``
# always returns the module.
app_module = importlib.import_module("aegiscore.sandbox_server.app")
app = app_module.app
...
def _fake_apply_result(detail: str):
    from aegiscore.sandbox_kernel.egress import EgressApplyResult
