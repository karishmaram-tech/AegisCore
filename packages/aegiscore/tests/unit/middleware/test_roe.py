...
from aegiscore.middleware import roe as roe_mod
from aegiscore.middleware._audit_sink import RoEAuditSink, verify_ledger
from aegiscore.middleware._command_targets import extract_targets
...
from aegiscore_core.types.roe import (
    EnforcementMode,
    MachineEnforcement,
    evaluate_command,
    evaluate_target,
...
class TestEgressProvisioningHook:
    ...
    def test_provisions_by_default_for_enforce_mode(self, tmp_path: Path, monkeypatch) -> None:
        ...
        from aegiscore.middleware.egress import EgressPolicy
    ...
    def test_opt_out_env_disables_provisioning(self, tmp_path: Path, monkeypatch) -> None:
        ...
        calls: list = []
        monkeypatch.setattr(
            "aegiscore.backends.factory.build_sandbox_backend",
    ...
    def test_retries_after_provision_failure(self, tmp_path: Path, monkeypatch) -> None:
        ...
        monkeypatch.setattr("aegiscore.backends.factory.build_sandbox_backend", lambda: _Flaky())
        mw = RoEGuardrailMiddleware()
        state = {"workspace_path": str(tmp_path)}
...
class TestSlotRegistration:
    ...
    def test_default_factory_is_registered(self) -> None:
        from aegiscore.agents.middleware_slots import DEFAULT_SLOT_FACTORIES
        from aegiscore_core.contracts.slots import MiddlewareSlot
