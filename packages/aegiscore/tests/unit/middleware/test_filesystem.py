...
from aegiscore.middleware.filesystem import (
...
def test_real_path_is_accepted_idempotently() -> None:
    ...
    visible on the host as ``~/.aegiscore/workspace/test/test/...``. The
...
def test_rebind_reresolves_bare_http_sandbox_per_run(monkeypatch) -> None:
    ...
    monkeypatch.setattr("aegiscore.backends.build_sandbox_backend", lambda config=None: per_run)
...
def test_rebind_leaves_composite_with_non_sandbox_default_untouched(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "aegiscore.backends.build_sandbox_backend",
