...
from aegiscore.middleware.engagement import (
...
from aegiscore.middleware.opplan import OPPLANState
...
def test_kg_state_fields_survive_parallel_fanout(field, value) -> None:
    ...
    from aegiscore.middleware.kg_internal.state import KGState
...
def test_reduce_converging_value_semantics() -> None:
    from aegiscore.middleware.state_reducers import reduce_converging_value
...
def test_deconfliction_identifiers_appear_in_injection(
    ...
    _write_deconfliction(
        tmp_path,
        {
            ...
            "identifiers": [
                ...
                {"type": "user-agent", "value": "aegiscore/1.0"},
            ],
        },
    )
    req = _FakeRequest(
    ...
    assert "user-agent: aegiscore/1.0" in text
...
def test_hydrate_pulls_engagement_from_configurable_when_state_empty(
    ...
    monkeypatch.setattr(
        "aegiscore.middleware.engagement._configurable_from_runnable_config",
...
def test_build_language_policy_helper_handles_aliases_and_no_op_cases() -> None:
    ...
    from aegiscore.agents.prompts import build_language_policy
