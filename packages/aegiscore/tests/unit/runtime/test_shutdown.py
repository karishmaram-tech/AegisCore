"""Tests for ``aegiscore.runtime.shutdown`` — bounded graceful shutdown.
...
def _write_driver(
    ...
    *,
    ...
    script.write_text(
        textwrap.dedent(
            f"""
            # Set BEFORE any aegiscore import — aegiscore/__init__.py runs
            ...
            from aegiscore.runtime.shutdown import install_shutdown_handlers
...
class TestFlushLogicInProcess:
    ...
    def test_run_flush_writes_finding_opplan_and_executive(self, tmp_path: Path) -> None:
        from aegiscore.runtime import shutdown as mod
