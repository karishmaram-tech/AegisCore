"""Tests for aegiscore.runtime.cart."""
...
from aegiscore.runtime import cart, recording
from aegiscore.runtime.cart import (
    ChangeEvent,
    EngagementSnapshot,
    LinearOPPLANAdapter,
    ReplayPlan,
...
from aegiscore.runtime.task_spec import SubAgentTaskSpec
...
def test_replay_runner_execute_live_dispatches_one_spec_per_objective():
    ...
    assert spec.agent_name == "aegiscore"
...
def test_make_replay_dispatcher_installs_replay_middleware(monkeypatch):
    ...
    assert captured["agent_name"] == "aegiscore"
...
def test_make_replay_dispatcher_no_record_path_yields_empty_middleware():
    ...
    dispatcher = make_replay_dispatcher(fake_invoke)
    spec = SubAgentTaskSpec(agent_name="aegiscore", prompt="live", replay_record_path=None)
    result = dispatcher(spec)
    assert result == {"ok": True}
