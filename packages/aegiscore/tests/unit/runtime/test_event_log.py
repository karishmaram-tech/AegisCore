"""Tests for ``aegiscore.runtime.event_log`` — append-only engagement log."""
...
from aegiscore.runtime.event_log import (
    EngagementEvent,
    EventLog,
    EventType,
    read_events,
...
def test_append_writes_one_jsonl_line_per_event(tmp_path: Path) -> None:
    ...
    log.append(EventType.AGENT_TURN, {"turn": 1}, agent="aegiscore")
    ...
    assert parsed[1]["agent"] == "aegiscore"
...
def test_engagement_event_from_json_line_round_trips(tmp_path: Path) -> None:
    event = EngagementEvent(ts=1.5, type="agent.turn", agent="aegiscore", payload={"k": "v"})
    line = event.to_json_line()
    parsed = EngagementEvent.from_json_line(line)
    assert parsed is not None
    assert parsed.ts == 1.5
    assert parsed.type == "agent.turn"
    assert parsed.agent == "aegiscore"
...
def test_for_workspace_writes_at_workspace_root(tmp_path: Path) -> None:
    ...
    assert events[1].agent == "aegiscore"
