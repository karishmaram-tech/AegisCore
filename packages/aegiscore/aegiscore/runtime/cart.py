...
the customer's infra changes, Aegiscore replays the last successful kill
...
- :class:`ReplayRunner` — orchestrates a replay using the existing
  :class:`aegiscore.runtime.recording.ReplayMiddleware`. When a snapshot
...
from aegiscore.runtime.recording import ReplayMiddleware
from aegiscore.runtime.task_spec import Dispatcher, SubAgentTaskSpec
...
class AttackGraphProtocol(Protocol):
    ...
    ``aegiscore.middleware.kg_internal.store``). CART consumes only
...
class ReplayRunner:
    ...
    produced (via :class:`aegiscore.runtime.recording.RecordingMiddleware`).
    ...
    def execute(self, plan: ReplayPlan) -> dict[str, Any]:
        ...
        # objective. agent_name is the orchestrator role ("aegiscore"); it
