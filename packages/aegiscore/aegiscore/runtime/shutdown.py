...
log = logging.getLogger("aegiscore.runtime.shutdown")
...
def _on_signal(signum: int | None, frame: Any) -> None:
    ...
    worker = threading.Thread(target=_worker, name="aegiscore-shutdown-flush", daemon=True)
    worker.start()
...
def _run_step(
    ...
    t = threading.Thread(target=_runner, name=f"aegiscore-shutdown-{label}", daemon=True)
    t.start()
    t.join(timeout=cap)
    if t.is_alive():
