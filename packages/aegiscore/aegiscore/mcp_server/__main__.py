"""``aegiscore-mcp`` / ``python -m aegiscore.mcp_server`` entrypoint.
...
_INSTALL_HINT = (
    "aegiscore-mcp requires the optional 'mcp' dependency. "
    "Install it with:  pip install 'aegiscore[mcp]'"
)
...
def _build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="aegiscore-mcp",
        description=(
            "Expose Aegiscore engagement control over MCP "
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        from aegiscore.mcp_server.auth import open_bind_error, resolve_auth_mode
    ...
    try:
        ...
        print(f"aegiscore-mcp: {exc}", file=sys.stderr)
        return 2
