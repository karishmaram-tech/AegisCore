"""Typed result models for the Aegiscore engagement MCP server.

These models are the structured payloads the MCP tools in
:mod:`aegiscore.mcp_server.server` return to external agent runtimes
...
#: Engagement depth/timeout profile. Mirrors ``aegiscore.cli.scan``'s
#: ``--scan-mode`` choices so the MCP bridge and the headless CLI agree.
ScanMode = Literal["quick", "standard", "deep"]


class GraphInfo(BaseModel):
    """A single engagement graph (assistant) exposed by the Aegiscore server."""
