...
the port drive Aegiscore. This builds the MCP SDK's :class:`TokenVerifier` for
...
Aegiscore depends on the official ``mcp`` SDK (``mcp.server.fastmcp``), not the
...
class SharedSecretVerifier(TokenVerifier):
    ...
    async def verify_token(self, token: str) -> AccessToken | None:
        ...
        return AccessToken(
            token=token,
            client_id="aegiscore-mcp:shared-secret",
            scopes=[],
