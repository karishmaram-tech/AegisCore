"""Drive Aegiscore engagements through the LangGraph SDK.

A thin async bridge: MCP tool calls are translated into LangGraph runs
against a running Aegiscore server, reusing the same scope/state payload
shape as ``aegiscore.cli.scan`` so the orchestrator (and its RoE
...
from aegiscore.mcp_server.config import ServerConfig
from aegiscore.mcp_server.models import (
    EngagementSummary,
    GraphInfo,
    RunHandle,
    ScanMode,
...
from aegiscore.mcp_server.streaming import watch_run
...
class EngagementClient:
    """Async bridge from MCP tool calls to a running Aegiscore LangGraph server.
