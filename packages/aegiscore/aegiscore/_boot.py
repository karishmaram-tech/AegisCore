"""Framework boot — runs once at ``import aegiscore`` time.
...
``aegiscore/__init__.py``. Plugin authors interact with the
...
from aegiscore_core.contracts.slots import SLOTS_PER_ROLE, MiddlewareSlot
from aegiscore_core.plugin_loader import load_plugin_role_specs
...
def _oss_skill_sources_for(role: str) -> tuple[str, ...]:
    ...
    ``aegiscore.agents.middleware_slots``) for the baseline portion
...
def _register_plugin_roles() -> None:
    """Walk ``aegiscore.roles`` entry-points and register each spec.

    Closes spec §8 gap #5 — plugins shipping ``RoleSpec`` under the
    new ``aegiscore.roles`` entry-point group are first-class citizens
...
def _push_role_resolutions() -> None:
    ...
    Fields populated:
      * ``middleware_stack`` — every slot in ``SLOTS_PER_ROLE[role]``,
        attributed to ``aegiscore`` as owner (plugin overrides change
      ...
      * ``overrides_applied`` — empty for pure-OSS; populated when
        ...
        ``_resolve_overrides``.
    """
    from aegiscore.agents.middleware_slots import skills_sources_for as _osss

    for role, slots in SLOTS_PER_ROLE.items():
        middleware_stack = tuple(
            MiddlewareInfo(slot=slot.value, name=slot.value, owner="aegiscore")
        ...
        # the ``aegiscore.skills`` entry-point group at boot time.
...
def run() -> None:
    ...
    logger.debug(
        "aegiscore framework boot complete (%d OSS + N plugin roles registered, %d resolutions)",
        len(SLOTS_PER_ROLE),
        len(SLOTS_PER_ROLE),
    )
