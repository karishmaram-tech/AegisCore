...
Bundle model
------------
Aegiscore's graph manifest is split into two bundles by
``aegiscore.graph_registry``:
...
from aegiscore.graph_registry import _BUNDLE_TO_GRAPHS  # private, but stable: bundle->graphs map
...
def _path_to_module(spec: str) -> tuple[str, str]:
    """Turn ``"./aegiscore/agents/plugins/vulnresearch.py:graph"`` into
    ``("aegiscore.agents.plugins.vulnresearch", "graph")``.
...
def _registered_graph_ids() -> set[str]:
    ...
    would make ``aegiscore.server.plugins_api`` un-importable from
...
app = FastAPI(
    title="Aegiscore plugin bundle API",
