...
1. **Aegiscore-specific system prompt** — Replaces the generic "Skills System"
...
``<role>.md`` and moved to ``aegiscore/agents/prompts/workflows/``;
``PromptBuilder`` concatenates them into the cacheable static prefix at
agent factory time. See ``aegiscore/agents/prompts/builder.py``.
...
# ── Aegiscore skill system prompt template ──────────────────────────────────
...
class SkillsMiddleware(BaseSkillsMiddleware):
    ...
    - Aegiscore-specific system prompt template
    ...
    ``aegiscore/agents/prompts/builder.py`` for the
