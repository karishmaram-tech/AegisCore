...
Aegiscore agents ingest the output of every tool call back into their own
...
from aegiscore.middleware._injection_detector import neutralize_special_tokens
...
def _skill_tool_names() -> frozenset[str]:
    ...
    names: set[str] = set()
    try:
        from aegiscore.tools.skills import build_load_skill_tool
    ...
    try:
        from aegiscore.middleware import skillogy
...
def _untrusted_tool_names() -> frozenset[str]:
    ...
    the ``aegiscore.middleware`` package __init__ imports this module
