...
The parser-side :class:`~aegiscore.middleware.roe.RoEGuardrailMiddleware`
...
``apply_egress``) live in ``aegiscore.sandbox_kernel.egress`` because
...
``aegiscore_core``. They are re-exported here for convenience. This
...
from aegiscore.sandbox_kernel.egress import (
    EgressPolicy,
    render_dns_allowlist,
    render_nftables,
)
from aegiscore_core.types.roe import EnforcementMode, MachineEnforcement, ScopeRule
