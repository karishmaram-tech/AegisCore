...
Replaces the broken ``aegiscore.tools.research._state.graph_transaction``
...
``aegiscore_core.utils.engagement_scope`` exposed is no longer load-
...
from aegiscore_core.utils.engagement_scope import is_valid_engagement_label
...
class KGStore:
    ...
    def snapshot(self, *, engagement: str):
        ...
        ``aegiscore.runtime``, which the middleware does not need to
        pull in just to construct a snapshot.
        """
        from aegiscore.runtime.cart import EngagementSnapshot, SnapshotNodeKey, _hash_snapshot
