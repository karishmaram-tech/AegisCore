...
# Files / dirs that must be clean. In Aegiscore the engine lives at
# aegiscore/sandbox_web/ (upstream insane-search used engine/).
...
EXPLICIT_ALLOW_FILES = {
    ...
    # Aegiscore layout: the engine lives at aegiscore/sandbox_web/, so the
    # sanctioned router resolves here. Both forms are kept so the linter works
    # whether run from the upstream skill tree or the Aegiscore package.
    "sandbox_web/phase0.py",
    "aegiscore/sandbox_web/phase0.py",
}
