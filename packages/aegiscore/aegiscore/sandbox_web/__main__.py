...
execution surface:
    ...
    python3 -m aegiscore.sandbox_web fetch  <url>   [opts] --json
    python3 -m aegiscore.sandbox_web search <query> [opts] --json
...
from aegiscore.sandbox_web.fetch_chain import ScopeCheck, fetch
from aegiscore.sandbox_web.providers import PROVIDERS, SearchResponse, is_allowed_provider
...
def _build_scope_check(workspace: str | None) -> ScopeCheck | None:
    ...
    if not roe_path.exists():
        return None
    try:
        from aegiscore_core.types.roe import MachineEnforcement, evaluate_target
    except ImportError:
        return None
    try:
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser = argparse.ArgumentParser(prog="aegiscore.sandbox_web", description="open-web engine")
