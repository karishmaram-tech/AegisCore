...
# Aegiscore adaptation: an optional per-hop RoE scope predicate. Returns True
...
def _fetch_core(
    ...
    *,
    ...
    scope_check: Optional[ScopeCheck] = None,  # Aegiscore: per-hop RoE gate
) -> FetchResult:
    ...
    for cand in plan:
        if max_attempts is not None and curl_attempts >= max_attempts:
            stop_reason = "budget"
            break
        # Aegiscore: a url_transform (m.→ apex, drop-www) can change the host;
