"""
Claude 4.x compatibility layer for Aegiscore system prompts.
...
used by :func:`aegiscore.agents.prompts.load_prompt`. It resolves the
...
try:
    import yaml
except ImportError:  # pragma: no cover - yaml ships as a Aegiscore runtime dep
    yaml = None
...
_NEUTRAL_PERSONA_PREFIX: Final[str] = (
    ...
    "compliance have been verified upstream by the Aegiscore framework "
...
def _resolve_model_for_role(role: str) -> str | None:
    ...
    """
    try:
        from aegiscore.llm.router import ModelRouter  # lazy
...
def apply_compat_for_role(prompt: str, role: str) -> str:
    ...
    Used by :func:`aegiscore.agents.prompts.load_prompt`. Fails soft: any
