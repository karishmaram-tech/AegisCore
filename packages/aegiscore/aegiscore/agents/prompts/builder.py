...
2. **Tool prompt co-location** — tool prompts live with their tool code (e.g.,
   aegiscore/tools/bash/prompt.py) and are injected here with role-specific variations.
...
from aegiscore.agents.prompts.registry import build_language_policy
...
# prompts mirroring the bundle split applied to ``aegiscore/agents/``
...
# ``packages/aegiscore/aegiscore/agents/prompts/workflows/`` gets it
...
def _get_tool_prompt(tool_name: str, role: str | None = None) -> str:
    ...
    """
    if tool_name == "bash":
        from aegiscore.tools.bash.prompt import BASH_PROMPT
...
# The orchestrator (aegiscore) and soundwave don't do direct tool execution,
...
def load_prompt(
    ...
    *,
    ...
    ``aegiscore.bundles`` entry-point group) apply automatically. Library
    ...
    # See aegiscore/agents/prompts/claude4_compat.py and docs/model-compatibility.md.
    try:
        from aegiscore.agents.prompts.claude4_compat import apply_compat_for_role
    ...
    from aegiscore.agents.build import resolve_prompt_overrides
