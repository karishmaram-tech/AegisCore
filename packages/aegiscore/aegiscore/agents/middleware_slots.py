...
Every Aegiscore agent factory composes its middleware stack from a
...
from aegiscore.middleware.model_override import ModelOverrideMiddleware
from aegiscore.middleware.notifications import SandboxNotificationMiddleware
from aegiscore.middleware.opscontrol_notifications import (
    OpsControlNotificationMiddleware,
)
from aegiscore.middleware.prompt_injection_shield import PromptInjectionShieldMiddleware
from aegiscore.middleware.proxy_key_override import ProxyKeyOverrideMiddleware
from aegiscore.middleware.roe import build_default_sink
from aegiscore.middleware.subagent_transcript_state import SubagentTranscriptState

# Slot enum + per-role applicability mapping + safety-critical set
# all live in the contract layer now (aegiscore_core.contracts.slots).
...
# framework call sites — ``aegiscore.agents.build`` and the test
# tree — keep working until Phase 2 rewrites them to import from
# ``aegiscore_core.contracts.slots`` directly.
from aegiscore_core.contracts.slots import (
    SAFETY_CRITICAL_SLOTS,
    SLOTS_PER_ROLE,
    MiddlewareSlot,
)
from aegiscore_core.plugin_loader import load_plugin_skill_sources
...
def skills_sources_for(role: str) -> list[str]:
    ...
    ``aegiscore/agents/_benchmark_mode.py``. Plugin-contributed paths
    (registered under the ``aegiscore.skills`` entry-point group) are
...
def _make_kg(**_: Any):
    ...
    """
    from aegiscore.middleware.kg_internal.store import (
        KGStoreConfigError,
...
see ``aegiscore.agents.build.build_middleware``."""


__all__ = [
    # Re-exported from aegiscore_core.contracts.slots so existing call
