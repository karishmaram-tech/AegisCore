...
Two override channels feed into the same pipeline:
  ...
  2. **Plugin packages** ship declarative ``PluginBundle`` objects under
     the ``aegiscore.bundles`` entry-point group. The bundle carries
...
from aegiscore.agents.middleware_slots import (
    DEFAULT_SLOT_FACTORIES,
    SAFETY_CRITICAL_SLOTS,
    SLOTS_PER_ROLE,
    MiddlewareSlot,
)
from aegiscore.middleware.skillogy import maybe_install_skillogy
...
from aegiscore_core.registry import SafetyRegistry
...
# ``aegiscore.callbacks`` groups stay for additive contributions; this
# new group is exclusively for plugins that need to disable or replace
# pieces of the standard stack.
BUNDLES_GROUP: str = "aegiscore.bundles"
...
def _iter_override_bundles(role: str) -> Iterator[PluginBundle]:
    ...
    ``aegiscore.bundles`` that is enabled AND scoped to ``role``."""
    try:
...
def _resolve_overrides(
    *,
    ...
    from aegiscore_core.registry import PluginConflictWarning
    ...
    *,
    ...
    The safety-critical set is the union of:
      ...
      * plugin-declared additions via
        ``aegiscore_core.registry.SafetyRegistry.register(decl, owner=...)``
    ...
    *,
    ...
    if slots is None:
        raise KeyError(
            ...
            f"to SLOTS_PER_ROLE in aegiscore/agents/middleware_slots.py "
            f"(OSS roles)."
        )
    role_slots = slots
...
def resolve_prompt_overrides(
    ...
    *,
    ...
    Used by ``load_prompt`` in ``aegiscore.agents.prompts`` — kept
    here so plugin / safety-gate logic lives next to the rest of the
    override machinery. Callers do NOT include ``aegiscore.agents.prompts``
