"""Aegiscore — autonomous red team coordinator agent.
...
cleanly:
  ...
  3. **Full custom** (library composer): import building blocks from
     ``aegiscore.middleware`` / ``aegiscore.tools`` and compose with
     ``langchain.agents.create_agent`` directly. Aegiscore's factory
...
from aegiscore.core.subagent_streaming import StreamingRunnable
...
def create_decepticon_agent(
    *,
    ...
    """Build the Aegiscore orchestrator.
    ...
    Args:
        ...
        llm: bound chat model. Defaults to
            ``LLMFactory().get_model("aegiscore")``.
        fallback_models: passed to ``ModelFallbackMiddleware``. Defaults
            to ``LLMFactory().get_fallback_models("aegiscore")``.
    ...
    # ``aegiscore.subagents`` entry-point group; this main agent picks
    # up every spec whose ``parent_agents`` includes ``"aegiscore"``.
    # Community or downstream plugin packages can extend this roster without
    # modifying OSS — see ``aegiscore/plugin_loader.py`` for the loader
    ...
    if tools is None:
        ...
        # present (``aegiscore start`` topology). Daemon-less topologies —
        ...
        from aegiscore.tools.ops import OPS_TOOLS, ops_available
