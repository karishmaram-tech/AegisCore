"""Skill loading tools for Aegiscore agents."""
...
from aegiscore.tools.skills_registry import (
    AmbiguousSkill,
    SkillRecord,
...
# ── load_skill tool ──────────────────────────────────────────────────────────
# A Aegiscore-specific replacement for `load_skill("/skills/...")` that
...
def _default_skill_registry() -> tuple[SkillRecord, ...]:
    ...
    """
    try:
        root = files("aegiscore").joinpath("skills")
...
def _read_via_backend(backend: Any, skill_path: str) -> tuple[str | None, str | None]:
    ...
    ``FilesystemBackend`` (the package's ``aegiscore/skills`` tree, read
...
def build_load_skill_tool(
    ...
    *,
    ...
    (see ``aegiscore/backends/__init__.py:make_agent_backend``), which
    ...
    def load_skill(skill_path: str, include_siblings: bool = False) -> str:
        """Load a Aegiscore skill file (full body, no line-limit truncation).
