"""complete_engagement_planning — signal soundwave-to-aegiscore handoff.
...
``soundwave`` to ``aegiscore`` so the next operator message lands on the
...
def complete_engagement_planning(
    ...
    """Signal that engagement planning is finished and hand off to Aegiscore.
    ...
    their schemas. The CLI will switch the active assistant to Aegiscore and
