...
(``aegiscore.sandbox_web``) **inside the sandbox** over the existing execution
...
from aegiscore.tools.bash.bash import _workspace_path_from_config, get_sandbox

# Engine invocation. ``python3 -m aegiscore.sandbox_web`` runs inside the
# sandbox image (which mounts aegiscore + carries curl_cffi); the verb/args
# follow aegiscore/sandbox_web/__main__.py.
_ENGINE = "python3 -m aegiscore.sandbox_web"
_FETCH_TIMEOUT = 90
_SEARCH_TIMEOUT = 60
