...
Aegiscore's graph dedups writes by deterministic node ID (SHA1 of kind +
canonical key — see :mod:`aegiscore_core.types.kg`). That collapses
...
- **Cheap-first**: a deterministic :func:`prefilter` (same
  :class:`~aegiscore_core.types.kg.NodeKind` plus normalized host /
...
from aegiscore.tools.research._state import _json, graph_transaction
from aegiscore_core.types.kg import KnowledgeGraph, Node, NodeKind
...
def kg_dedupe_findings(min_cluster_size: int = 2) -> str:
    ...
    different agents (recon / exploit / analyst / vulnresearch). Aegiscore
