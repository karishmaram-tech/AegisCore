...
The inverse of :mod:`aegiscore.tools.research.sarif` (which ingests SARIF
...
Mapping
-------
Aegiscore Finding → SARIF result. Vulnerability/CVE/Weakness nodes
...
Severity:
  ...
  Aegiscore → SARIF level     → security-severity
  critical    error             10.0
  high        error              7.0
  medium      warning            5.0
  low         note               3.0
...
_TOOL_NAME = "Aegiscore"
_TOOL_INFORMATION_URI = "https://aegiscore.red"
...
def _finding_rule_id(node: Any, props: dict[str, Any]) -> str:
    ...
    if cve:
        return f"aegiscore/{cve}"
    if cwe:
        return (
            f"aegiscore/CWE-{cwe}"
            if not str(cwe).upper().startswith("CWE-")
            else f"aegiscore/{cwe}"
        )
    if klass:
        return f"aegiscore/{klass}"
    if technique:
        return f"aegiscore/{technique}"
...
def _iter_findings(graph: Any):
    ...
    Accepts both aegiscore_core ``NodeKind`` enum values and string kinds
    on the input nodes; classification is by lowercase string equivalence
    so duck-typed test doubles work without importing aegiscore_core.
    """
...
def export_findings_to_sarif(
    graph: Any,
    *,
    engagement_name: str = "Aegiscore Engagement",
    ...
    pull in the heavyweight aegiscore_core import path until call time.
    """
    ...
    for node, props in _iter_findings(graph):
        ...
        result: dict[str, Any] = {
            ...
            "properties": {
                "security-severity": f"{security_severity:.1f}",
                "aegiscore-finding-id": getattr(node, "id", None),
                "aegiscore-engagement": engagement_name,
            },
        }
        ...
        "runs": [
            {
                ...
                "properties": {
                    "aegiscore-engagement": engagement_name,
                    "aegiscore-finding-count": len(results),
                },
            }
        ],
    }
...
def severity_threshold_breach(document: dict[str, Any], *, fail_on: str = "high") -> bool:
    ...
    Used by CI gates: ``aegiscore scan ... --fail-on high`` → exit non-zero
