...
``aegiscore.tools.research._state`` — AD_TOOLS, CONTRACT_TOOLS,
RESEARCH_TOOLS, ``chain.py``, ``health.py``, ``dedupe.py`` — now sits
on top of the new ``KGStore`` (``aegiscore.middleware.kg_internal.store``)
...
What the shim guarantees:
  ...
  - Engagement scoping is read from the
    ...
    (``aegiscore_core.utils.engagement_scope.get_active_engagement``).
...
from aegiscore.tools.reporting.kg_adapter import load_engagement_graph
from aegiscore.tools.research._engagement_scope import _LEGACY_ENGAGEMENT_LABEL
from aegiscore_core.types.kg import Edge, KnowledgeGraph, Node
