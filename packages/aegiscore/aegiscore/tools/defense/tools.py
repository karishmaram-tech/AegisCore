...
Each tool follows the Aegiscore convention: returns a JSON string so the
...
from aegiscore.tools.defense import edr, elastic, sentinel, splunk
...
def sigma_to_splunk_savedsearch(
    ...
    The rule name will be prefixed with ``aegiscore-eng-<slug>::`` so the
...
def sigma_to_sentinel_analyticrule(
    ...
    onto a ``aegiscore-eng-<slug>`` prefix for blue-team auditability.
    """
    try:
        rule_dict = _coerce_sigma(sigma_rule)
    except Exception as exc:  # noqa: BLE001
