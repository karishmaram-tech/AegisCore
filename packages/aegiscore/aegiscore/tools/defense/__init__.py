...
- ConOps must declare the SIEM endpoint (URL + auth method) under
  ...
  ConOps key.
- Every pushed rule carries a Aegiscore-scoped tag prefix
  (``aegiscore-eng-<slug>``) so blue-team operators can identify and
...
from aegiscore.tools.defense.tools import DEFENSE_TOOLS
