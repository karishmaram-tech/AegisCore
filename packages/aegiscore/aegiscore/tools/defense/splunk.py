...
in-house translator that handles the subset Aegiscore's Detector agent
...
from aegiscore.tools.defense.conops import (
    ConOpsLookupError,
    engagement_slug,
    resolve_auth_value,
    resolve_siem_target,
...
def push_savedsearch(
    ...
    *,
    ...
    Tags the saved search ``aegiscore-eng-<slug>`` so blue-team can audit
    ...
    tag = f"aegiscore-eng-{engagement_slug()}"
    ...
    payload = {
        ...
        "description": (
            f"{description}\n\n"
            f"Created by Aegiscore engagement ``{engagement_slug()}``"
