...
def _ollama_cloud_configured() -> bool:
    ...
    `aegiscore onboard` (relying on the default base + model) is still
...
def _reraise_if_connection_error(exc: Exception) -> None:
    ...
    if any(
        ...
        raise RuntimeError(
            ...
            f"Check 'aegiscore logs litellm' for details."
        ) from exc
...
def _reraise_with_actionable_message(exc: Exception, model_name: str) -> None:
    ...
    if (
        ...
        raise RuntimeError(
            ...
            f"Check the API key for that provider in ~/.aegiscore/.env, "
            f"or run 'aegiscore onboard --reset'.\nUnderlying: {safe_msg}"
        ) from exc
...
class LLMFactory:
    ...
    def get_model(self, role: str, *, default_role: str | None = None) -> BaseChatModel:
        ...
        e.g. ``LLMFactory().get_model("aegiscore-pro", default_role="aegiscore")``.

        When the caller leaves ``default_role`` unset, the factory consults
        ``aegiscore_core.registry.RoleRegistry`` for the role's registered
        ...
        site to thread ``default_role="aegiscore"`` through manually
        (closes spec §8 gap #5).
        """
