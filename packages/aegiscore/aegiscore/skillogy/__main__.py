"""``python -m aegiscore.skillogy`` — run the Skillogy server.
...
Environment variables:
  ...
  SKILLOGY_NEO4J_PASSWORD     (default ``aegiscore-graph``)
...
from aegiscore.skillogy.server.app import build_app
from aegiscore.skillogy.server.neo4j_backend import Neo4jBackend
...
def _build_backend() -> Neo4jBackend:
    return Neo4jBackend(
        ...
        password=os.environ.get("SKILLOGY_NEO4J_PASSWORD", "aegiscore-graph"),
    )
...
def _maybe_ingest(backend: Neo4jBackend) -> None:
    ...
    from aegiscore.skillogy.embed_ingest import ingest_embeddings  # noqa: PLC0415
...
def _start_rest(backend: Neo4jBackend, port: int, started_at: float) -> None:
    ...
    # aegiscore-net; docker-compose pins the host port to 127.0.0.1.
