"""``python -m aegiscore.skillogy.builder`` — assemble the full
...
from aegiscore.skillogy.builder.emit import emit_cypher
from aegiscore.skillogy.builder.mitre_stix import emit_mitre_records
from aegiscore.skillogy.builder.model import Edge, Node
from aegiscore.skillogy.builder.seeds_to_graph import emit_all_seed_records
from aegiscore.skillogy.builder.skills import emit_skill_records
...
def _build_argparser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="python -m aegiscore.skillogy.builder",
        description="Compile SKILL.md + seeds + MITRE STIX into skills.cypher.",
    )
    parser.add_argument(
        "--skills-root",
        type=Path,
        default=Path("packages/aegiscore/aegiscore/skills"),
    ...
    parser.add_argument(
        "--out",
        type=Path,
        default=Path("packages/aegiscore/aegiscore/skills/.graph/skills.cypher"),
...
def main(argv: list[str] | None = None) -> ExitCode:
    ...
    if args.check:
        ...
        print(
            ...
            f"Run `python -m aegiscore.skillogy.builder` to regenerate.",
            file=sys.stderr,
        )
        return ExitCode.DIFF_FOUND
