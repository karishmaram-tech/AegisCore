...
this module ships the one-release migration shim:
  ...
  * Legacy import paths (``aegiscore.core.schemas``,
    ``aegiscore.llm.models``, ``aegiscore.tools.research.graph``,
    ``aegiscore.plugin_loader``, ``aegiscore.core.config``,
...
``aegiscore_core.*`` directly; downstream consumers we don't know
...
_LEGACY_PATH_NOTES: tuple[tuple[str, str], ...] = (
    ...
    (
        ...
        "aegiscore_core.contracts.slots (enum/SLOTS_PER_ROLE only)",
    ),
)
...
def register_legacy_imports() -> None:
    ...
    lines = ["Aegiscore Phase 1 compat shims active. Migrate the following paths before 2.0.0:"]
