...
log = logging.getLogger("aegiscore.telemetry.otel")
...
def _service_name() -> str:
    return os.getenv("OTEL_SERVICE_NAME", "aegiscore")
...
def init_otel() -> bool:
    ...
    _TRACER = trace.get_tracer("aegiscore")
    _INITIALIZED = True
    return True
...
def start_engagement_span(engagement_id: str) -> Iterator[Any]:
    ...
    with tracer.start_as_current_span("aegiscore.engagement") as span:
        span.set_attribute("aegiscore.engagement_id", engagement_id)
...
def start_agent_span(agent_name: str) -> Iterator[Any]:
    ...
    with tracer.start_as_current_span("aegiscore.agent_run") as span:
        span.set_attribute("aegiscore.agent", agent_name)
        eng_id = _current_engagement_id.get()
        if eng_id:
            span.set_attribute("aegiscore.engagement_id", eng_id)
        try:
            yield span
        finally:
            _current_agent.reset(agent_token)
...
def start_tool_span(tool_name: str) -> Iterator[Any]:
    ...
    with tracer.start_as_current_span("aegiscore.tool_call") as span:
        span.set_attribute("aegiscore.tool", tool_name)
        eng_id = _current_engagement_id.get()
        if eng_id:
            span.set_attribute("aegiscore.engagement_id", eng_id)
        agent = _current_agent.get()
        if agent:
            span.set_attribute("aegiscore.agent", agent)
        objective = _current_objective_id.get()
        if objective:
            span.set_attribute("aegiscore.opplan.objective_id", objective)
        yield span
...
def start_llm_span(model: str) -> Iterator[Any]:
    ...
    with tracer.start_as_current_span("aegiscore.llm_call") as span:
        span.set_attribute("aegiscore.llm.model", model)
        eng_id = _current_engagement_id.get()
        if eng_id:
            span.set_attribute("aegiscore.engagement_id", eng_id)
        agent = _current_agent.get()
...
def record_llm_cost(cost_usd: float | None) -> None:
    ...
    if llm_span is not None and _set_span_attribute(
        llm_span, "aegiscore.llm.cost_usd", float(cost_usd)
    ):
        return
    eng_span = _active_engagement_span.get()
    if eng_span is not None:
        _set_span_attribute(eng_span, "aegiscore.llm.cost_usd", float(cost_usd))
...
def record_llm_token_usage(
    ...
    if prompt_tokens is not None:
        _set_span_attribute(span, "aegiscore.llm.prompt_tokens", int(prompt_tokens))
    if completion_tokens is not None:
        _set_span_attribute(span, "aegiscore.llm.completion_tokens", int(completion_tokens))
