...
# Skills ship as package data under ``aegiscore/skills/`` and are read
...
# and the langgraph Docker image (``/app/aegiscore/skills``) — so no
# container-specific path is hardcoded.
SKILLS_LOCAL_PATH = str(importlib.resources.files("aegiscore") / "skills")
...
def make_agent_backend(
    ...
    *,
    ...
    """Compose the runtime backend for a Aegiscore agent.

    Routes ``/skills/`` to a local ``FilesystemBackend`` reading the
    package's ``aegiscore/skills`` tree in-process, and routes everything
    ...
    backend object while reading from different physical storage:
      ...
      /skills/...   ->  aegiscore/skills/... read in-process (~5ms)
