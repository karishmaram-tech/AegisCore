...
`aegiscore.sandbox_server`, a FastAPI app that runs *inside* the
...
from aegiscore.sandbox_kernel import BackgroundJob, BackgroundJobTracker

if TYPE_CHECKING:
    from aegiscore.sandbox_kernel.egress import EgressPolicy
...
class HTTPSandbox(BaseSandbox):
    """A `BaseSandbox` that talks to a `aegiscore.sandbox_server` daemon.
    ...
    def _http(self) -> httpx.Client:
        ...
        if self._client is None:
            headers = {"User-Agent": "aegiscore-http-sandbox/1"}
            if self._token:
    ...
    # `aegiscore/tools/bash/bash.py:_sandbox.execute_tmux_async(...)`
