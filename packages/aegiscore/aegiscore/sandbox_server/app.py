...
Layering: the daemon imports only from `aegiscore.sandbox_kernel.*` so
...
log = logging.getLogger("aegiscore.sandbox_server")
...
class ProvisionEgressRequest(BaseModel):
    policy: dict = Field(
        ...
        "(aegiscore.sandbox_kernel.egress.EgressPolicy.to_dict()): the "
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    # next engagement attempt. Scoped to aegiscore-named sockets
...
app = FastAPI(
    title="Aegiscore sandbox daemon",
...
def provision_egress(
    ...
    """
    from aegiscore.sandbox_kernel.egress import EgressPolicy, apply_egress
