...
no ``aegiscore_core``, no langchain.

The agent side (``aegiscore.middleware.egress``) compiles the
...
class EgressPolicy:
    ...
    Plain stdlib dataclass (no ``aegiscore_core`` dependency) so it can
...
def render_nftables(
    ...
    *,
    ...
    lines = [
        "#!/usr/sbin/nft -f",
        "# Aegiscore RoE egress guardrail — generated; do not hand-edit.",
