...
from aegiscore.sandbox_kernel.jobs import BackgroundJob, BackgroundJobTracker
from aegiscore.sandbox_kernel.tmux import (
    PS1_PATTERN,
    TmuxSessionManager,
    _safe_log,
...
log = logging.getLogger("aegiscore.sandbox_kernel.base")
...
def _compute_orphan_tmux_sessions(
    ...
    Returns the subset of ``discovered`` that are aegiscore-owned
    (``dcptn_`` prefix) AND not present in ``tracked``. Tracked sessions
    are spared; non-aegiscore-prefixed names are spared regardless.
    """
    return {
        name
...
def _list_decepticon_tmux_sockets() -> list[str]:
    ...
    matching the aegiscore prefix so we never touch an unrelated user's
...
def _kill_tmux_socket(session: str) -> None:
    """Kill the tmux server backing a single aegiscore socket.
...
class SandboxBase(BaseSandbox):
    ...
    def __init__(
        self,
        container_name: str = "aegiscore-sandbox",
    ...
    def reap_orphaned_tmux_sessions(self) -> int:
        """Kill aegiscore-named tmux sockets this process does NOT own.
