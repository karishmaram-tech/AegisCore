...
``DaemonSandbox`` class that the daemon wraps. Two consumers hang off it:
  ...
  ``aegiscore.backends.http_sandbox.HTTPSandbox`` — agent-side,
  ...
  ``aegiscore.sandbox_server.app`` — sandbox-side, FastAPI daemon.
...
zero aegiscore Python code. Adding the daemon required *some* in-
