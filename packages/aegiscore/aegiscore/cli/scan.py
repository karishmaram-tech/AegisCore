"""``aegiscore-cli scan`` — headless / CI security scan entry.

Modeled after Strix's CLI for surface familiarity, with Aegiscore's
engagement / OPPLAN / RoE discipline preserved:

  aegiscore-cli scan --target ./ --scan-mode quick \
      --sarif-output aegiscore.sarif --fail-on high
...
The actual scan is driven by the existing ``aegiscore`` orchestrator via
...
def _build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(
        prog="aegiscore-cli scan",
        description="Run a Aegiscore security scan and emit SARIF for CI.",
    )
    p.add_argument(
        "--target",
        "-t",
    ...
    p.add_argument(
        "--assistant",
        default="aegiscore",
        help="LangGraph assistant graph name (default: aegiscore).",
    )
    p.add_argument(
        "--engagement-name",
        default=None,
...
def _load_findings_graph(engagement_name: str) -> Any | None:
    ...
    if not graph_path.exists():
        ...
        return None
    try:
        from aegiscore_core.types.kg import KnowledgeGraph  # noqa: PLC0415
...
def _write_sarif_and_gate(
    ...
    from aegiscore.tools.research.sarif_export import (  # noqa: PLC0415
