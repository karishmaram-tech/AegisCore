"""``aegiscore-cli telemetry`` — inspect and control usage telemetry.
...
from aegiscore.telemetry.config import (
    TelemetryConfig,
    TelemetryMode,
    is_opted_out,
    resolve_config,
...
from aegiscore.telemetry.sink import TelemetrySink

# Plain-language disclosure shown on `enable` — the OSS consent contract.
_CONSENT_NOTICE = """\
Aegiscore usage telemetry — what you are consenting to share:
...
Run `aegiscore-cli telemetry preview` to see the exact payload before sending.
