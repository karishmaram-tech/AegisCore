"""``aegiscore-cli auth`` — headless provider/auth introspection.

The interactive ``aegiscore onboard`` wizard (Go launcher) is the place to
...
if TYPE_CHECKING:
    from aegiscore.llm.factory import AuthInventory
...
def _render_text(inv: AuthInventory) -> str:
    lines: list[str] = ["Aegiscore auth status", "=" * 22, ""]
    ...
    if inv.resolved_chain:
        ...
        lines.append("  Run `aegiscore onboard` (or set an API key / subscription) — every")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        from aegiscore.llm.factory import auth_inventory  # noqa: PLC0415
