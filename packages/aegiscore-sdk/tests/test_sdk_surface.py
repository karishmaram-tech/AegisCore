"""aegiscore-sdk surface tests.

Verifies the single-import contract from spec §6.3: a complete plugin
can be written importing only from ``aegiscore_sdk``. Tests that
...
def test_contributions_importable() -> None:
    ...
    assert SubAgentContribution(parent_agents=("aegiscore",)).items == ()


def test_fake_backend_satisfies_protocol() -> None:
    from aegiscore_sdk import BackendProtocol
    from aegiscore_sdk.testing import FakeBackend
...
def test_fake_llm_satisfies_protocol() -> None:
    from aegiscore_sdk import LLMProtocol
    from aegiscore_sdk.testing import FakeLLM
...
def test_fake_sandbox_satisfies_protocol() -> None:
    from aegiscore_sdk import SandboxProtocol
    from aegiscore_sdk.testing import FakeSandbox
...
def test_role_registry_idempotent() -> None:
    ...
    from aegiscore_sdk import MiddlewareSlot, RoleRegistry
...
def test_skill_source_registry_validates() -> None:
    from aegiscore_sdk import SkillSourceRegistry
