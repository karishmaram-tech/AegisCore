...
``pytest packages/aegiscore/tests`` requires the live sandbox HTTP
...
Lives under aegiscore-sdk/tests so it shares the existing pytest
testpaths (the aegiscore-sdk testing fakes already pull the
...
def test_compat_register_legacy_imports_idempotent() -> None:
    ...
    from aegiscore.compat import register_legacy_imports
...
def test_role_registry_seeded_with_16_oss_roles() -> None:
    ...
    import aegiscore  # boot fires

    del aegiscore  # silence unused-import lint

    from aegiscore_core.contracts.slots import SLOTS_PER_ROLE
    from aegiscore_core.registry import RoleRegistry
...
def test_plugin_registry_finds_oss_subagents() -> None:
    ...
    from aegiscore_core.registry import PluginRegistry
    ...
    # 13 OSS subagents register via ``aegiscore.subagents`` per
    # packages/aegiscore/pyproject.toml entry-points.
    subagent_plugins = [p for p in plugins if "aegiscore.subagents" in p.groups]
