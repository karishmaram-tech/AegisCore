"""``aegiscore-sdk plugin new`` — typer-based scaffold CLI.
...
``aegiscore.<group>`` entry-point group.

Sample run::

    aegiscore-sdk plugin new --kind=middleware --name=my-plugin --path=./my-plugin
    cd my-plugin
    uv build
...
from aegiscore_sdk.scaffold.templates import TEMPLATES, pyproject_for

app = typer.Typer(
    name="aegiscore-sdk",
    help="Aegiscore plugin author SDK — scaffold + utilities.",
...
def plugin_new(
    ...
    name: str = typer.Option(
        ...,
        "--name",
        help="Plugin name (PyPI-style, e.g. 'my-aegiscore-plugin').",
    ),
    ...
    """Scaffold a new Aegiscore plugin package."""
    ...
    typer.echo(f"created aegiscore plugin '{name}' (kind={kind}) at {target}")
