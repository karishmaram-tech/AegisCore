# Threat Model — Aegiscore as a Target

> Aegiscore is an autonomous offensive tool. That makes it a high-value
> target. This document is the STRIDE walk of Aegiscore's own attack
...
1. **Operator plane.** The human running Aegiscore, their workstation,
   their Claude/OpenAI/Codex OAuth tokens, the `.env` file with API
   keys.
2. **Management plane.** Everything on `aegiscore-net`: LangGraph,
...
| **Plugin entry-points** | Arbitrary Python on `import aegiscore` | `pip install` of one malicious bundle → agent-startup RCE |
...
| **Tampering** | Cypher injection via interpolated label | `_apoc_safety.ensure_safe` + parameter-bound queries | Done ([apoc-safety](../../packages/aegiscore/aegiscore/tools/research/_apoc_safety.py)) |
...
| Threat | Vector | Mitigation | Status |
|--------|--------|-----------|--------|
| **Spoofing** | Default master key `sk-aegiscore-master` documented | `.env.example` warns; web health route refuses to fall back | Partial — operator must rotate |
...
| **Elevation of privilege** | Compromised orchestrator dispatches arbitrary subagents | SubAgent allowlist via `aegiscore.subagents` entry-point group | Partial — relies on operator trust of installed plugins |
...
| **Information disclosure** | `engagements/[id]/graph` fell back to public Neo4j password | Refuses to query with `aegiscore-graph` | Done |
| **Information disclosure** | Health route hardcoded `LITELLM_API_KEY = "sk-aegiscore-master"` | Removed; refuses without explicit env | Done |
...
- **OpenAI/Anthropic/Google provider trust**: Aegiscore trusts the upstream LLM providers' published security posture.
