...
| Target | Description |
|--------|-------------|
| `make dogfood` | Full OSS UX on local code: builds the Go launcher + every service image (`:dev` tag), wires up an isolated `$AEGISCORE_HOME` under `.dogfood/`, then runs the launcher. The onboard wizard, engagement picker, `compose up`, health checks, and CLI all execute exactly as a `curl \| bash` install. |
...
- Two moving channel tags (Claude-Code-style soak model — both **final-only**):
  ...
  - GHCR `stable` → moved to the newest final release **baked ≥ the soak window** (default 7 days), by the **scheduled** `promote-stable.yml` workflow — *not* on release. The conservative default; the `${AEGISCORE_VERSION:-stable}` compose fallback resolves here.
