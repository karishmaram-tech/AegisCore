# Working Together on Aegiscore

This document describes how we collaborate day-to-day on the Aegiscore OSS
...
Aegiscore is developed on a **single trunk (`main`)** using **GitHub
...
- **External contributors** — anyone with a GitHub account. Contribute via
  ...
  branches directly to `PurpleAILAB/Aegiscore`.
...
- **Cosign keyless signing** — every GHCR image and multi-arch manifest
  ...
  cosign verify ghcr.io/purpleailab/<image>:<version> \
    --certificate-identity-regexp 'https://github\.com/PurpleAILAB/Aegiscore/' \
