...
| 5 | HIGH | `AEGISCORE_WORKSPACE_PATH=/workspace` set only in dev override; OSS sandbox missing it | **Already addressed.** The base `docker-compose.yml`'s `sandbox` service binds `${AEGISCORE_ENGAGEMENT_WORKSPACE:-${AEGISCORE_HOME:-~/.aegiscore}/workspace}` to `/workspace`. The env var is implicit through the bind mount, not a separate declaration. |
