...
|---|---|---|---|
| **오퍼레이터 (사람)** | 전략 수립, 명령 결정 | **LLM Agent** (Aegiscore/Recon/Exploit/PostExploit) | 사람 대신 LLM이 판단 |
| **미션 컨트롤** | 작전 계획, 목표 관리 | **Ralph Loop** (Aegiscore Orchestrator) | opplan.json 기반 자동 오케스트레이션 |
...
graph LR
    ...
    style aegiscore-net fill:#1a1a2e,stroke:#e94560,color:#fff
    style sandbox-net fill:#0f3460,stroke:#e94560,color:#fff
```

- **aegiscore-net**: 인프라 서비스 전용. 외부 포트는 `127.0.0.1`에만 바인딩
...
```mermaid
graph TD
    subgraph Orchestrator["Aegiscore Orchestrator (Opus 4.7)"]
...
| 항목 | 값 |
|------|-----|
| 이미지 | `aegiscore-sandbox` (Kali rolling) |
| 메모리 | 4GB |
| CPU | 2코어 |
| PID 제한 | 1024 |
| 네트워크 | `sandbox-net` |
...
| 항목 | 값 |
|------|-----|
| 이미지 | `aegiscore-c2-sliver` (Kali rolling) |
| Profile | `c2-sliver` |
| 메모리 | 2GB |
| CPU | 1코어 |
| PID 제한 | 512 |
...
| 서비스 | 포트 | 바인딩 | 네트워크 |
|--------|------|--------|----------|
| LangGraph API | 2024 | 127.0.0.1 | aegiscore-net |
| LiteLLM Proxy | 4000 | 127.0.0.1 | aegiscore-net |
| PostgreSQL | 5432 | 127.0.0.1 | aegiscore-net |
